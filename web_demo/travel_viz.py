...
# min_ts = datetime.strptime(min(travel_data["date"]), "%Y-%m-%d %H:%M :%S.%f")
# max_ts = datetime.strptime(max(travel_data["date"]), "%Y-%m-%d %H:%M :%S.%f") # :%S.%f
...
# images_=count = st.sidebar.number_input("Images to Show", value=10)
